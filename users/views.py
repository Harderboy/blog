...
class RegisterView(View):
    ...
    def post(self, request):
        ...
        # 跳转到首页
        # reverse 可以通过 namespace:name 来获取到视图对应的路由
        response = redirect(reverse('home:index'))
        # 设置cookie 以方便首页中用户信息展示的判断和用户信息的展示
        # 登录状态，会话结束后自动过期
...
class LoginView(View):
    ...
    def post(self, request):
        ...
        # 认证登录用户
        # 采用系统自带的认证方法进行认证，账号密码都正确返回一个user，否则返回一个None
        # 默认的认证方法是针对于 username 字段进行用户名的判断
        # 认证字段已经在 User 模型中的 USERNAME_FIELD = 'mobile' 修改
        user = authenticate(mobile=mobile, password=password)  # 13022364644
        ...
        # 设置状态保持的周期
        if remember != 'on':
            # 没有记住用户：set_expiry(0) 0代表浏览器会话结束就过期
            request.session.set_expiry(0)
            # 设置cookie
            response.set_cookie('is_login', True)
            response.set_cookie('username', user.username, max_age=30 * 24 * 3600)
        else:
            # 记住用户：set_expiry(None) None表示两周后过期
